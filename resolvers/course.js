const { client, course, batch } = require("../mongoDB");
...
const resolvers = {
  ...
  Mutation: {
      ...
      if (req.isAuth) {
        if ((req.userType = "admin")) {
          try {
            ...
            const batchId = new mongoDB.ObjectId(args.input.batch);
            const batchData = await batch.updateOne(
              { _id: batchId },
              { $push: { course: result.insertedId.toString() } }
            );
            return {
              msg: "data Added",
            };
    ...
    deleteCourse: async (_, args, { req }) => {
      if (req.isAuth) {
        try {
          const id = new mongoDB.ObjectId(args.courseID);
